...
if($test==false&&$test2==false&&$test3==false&&$text4==false){
if($username==""){
	echo "<script>alert('昵称不能为空哦');location='../lyb.php'</script>";
}else{
	if($comment==""){
	echo "<script>alert('理由不能为空哦');location='../lyb.php'</script>";
}else{
	echo $comment;
include"config.php";
...
echo "<script>alert('提交成功');location='../lyb.php'</script>";
}
